...
//	                                 &window, &renderer);
	...
	charset = SDL_LoadBMP("./assets/misc/cs8x8.bmp");
	if (charset == NULL) {
		printf("SDL_LoadBMP(cs8x8.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	SDL_SetColorKey(charset, true, 0x000000);
	eti = SDL_LoadBMP("./assets/misc/eti.bmp");
	if (eti == NULL) {
		printf("SDL_LoadBMP(eti.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	title = SDL_LoadBMP("./assets/misc/title.bmp");
	if (title == NULL) {
		printf("SDL_LoadBMP(title.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	zbycholud = SDL_LoadBMP("./assets/misc/zbycholud.bmp");
	if (zbycholud == NULL) {
		printf("SDL_LoadBMP(zbycholud.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	shot = SDL_LoadBMP("./assets/shots/shot.bmp");
	if (shot == NULL) {
		printf("SDL_LoadBMP(shot.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	shotplayer = SDL_LoadBMP("./assets/shots/shotplayer.bmp");
	if (shotplayer == NULL) {
		printf("SDL_LoadBMP(shotplayer.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	playership = SDL_LoadBMP("./assets/spaceship/spaceship.bmp");
	if (playership == NULL) {
		printf("SDL_LoadBMP(spaceship.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	playershipLeft = SDL_LoadBMP("./assets/spaceship/spaceshipLeft.bmp");
	if (playership == NULL) {
		printf("SDL_LoadBMP(spaceshipLeft.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	playershipRight = SDL_LoadBMP("./assets/spaceship/spaceshipRight.bmp");
	if (playership == NULL) {
		printf("SDL_LoadBMP(spaceshipRight.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	playershipForward = SDL_LoadBMP("./assets/spaceship/spaceshipForward.bmp");
	if (playership == NULL) {
		printf("SDL_LoadBMP(spaceshipForward.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	playershipBackward = SDL_LoadBMP("./assets/spaceship/spaceshipBackward.bmp");
	if (playership == NULL) {
		printf("SDL_LoadBMP(spaceshipBackward.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	heal = SDL_LoadBMP("./assets/hp/heal.bmp");
	if (heal == NULL) {
		printf("SDL_LoadBMP(heal.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	heart = SDL_LoadBMP("./assets/hp/serduszko.bmp");
	if (heart == NULL) {
		printf("SDL_LoadBMP(serduszko.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	heartHalf = SDL_LoadBMP("./assets/hp/serduszkopol.bmp");
	if (heartHalf == NULL) {
		printf("SDL_LoadBMP(serduszkopol.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	heartEmpty = SDL_LoadBMP("./assets/hp/serduszkonic.bmp");
	if (heartEmpty == NULL) {
		printf("SDL_LoadBMP(serduszkonic.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	background = SDL_LoadBMP("./assets/misc/background.bmp");
	if (background == NULL) {
		printf("SDL_LoadBMP(background.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	// wczytywanie grafik ocen
	gradeA = SDL_LoadBMP("./assets/grades/animatedA.bmp");
	if (gradeA == NULL) {
		printf("SDL_LoadBMP(animatedA.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeAPlus = SDL_LoadBMP("./assets/grades/animatedAPlus.bmp");
	if (gradeAPlus == NULL) {
		printf("SDL_LoadBMP(animatedAPlus.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeB = SDL_LoadBMP("./assets/grades/animatedB.bmp");
	if (gradeB == NULL) {
		printf("SDL_LoadBMP(animatedB.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeBPlus = SDL_LoadBMP("./assets/grades/animatedBPlus.bmp");
	if (gradeBPlus == NULL) {
		printf("SDL_LoadBMP(animatedBPlus.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeC = SDL_LoadBMP("./assets/grades/animatedC.bmp");
	if (gradeC == NULL) {
		printf("SDL_LoadBMP(animatedC.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeCPlus = SDL_LoadBMP("./assets/grades/animatedCPlus.bmp");
	if (gradeCPlus == NULL) {
		printf("SDL_LoadBMP(animatedCPlus.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeD = SDL_LoadBMP("./assets/grades/animatedD.bmp");
	if (gradeD == NULL) {
		printf("SDL_LoadBMP(animatedD.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeDPlus = SDL_LoadBMP("./assets/grades/animatedDPlus.bmp");
	if (gradeDPlus == NULL) {
		printf("SDL_LoadBMP(animatedDPlus.bmp) error: %s\n", SDL_GetError());
		returningEr(screen, scrtex, window, renderer);
	};
	gradeF = SDL_LoadBMP("./assets/grades/animatedf.bmp");
	if (gradeF == NULL) {
